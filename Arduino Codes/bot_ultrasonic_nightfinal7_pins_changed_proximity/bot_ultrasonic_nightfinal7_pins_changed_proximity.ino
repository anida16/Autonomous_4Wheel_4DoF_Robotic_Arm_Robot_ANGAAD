...
void backwardMotion() {
  ...
  while (1) {
    ...
    //
    //Serial.println("Distance in back Sensor = ");
    //Serial.print(distance1);
    //
    //          WARNING - COMMENT THE SECTION ABOVE BEFORE TESTING WITH JETSON
    //

    //startmotor();
    if (digitalRead(BRS) && digitalRead(BLS))    // Move Reverse //Back Right Hall = 1 and Back Left Hall = 1
    {
      backward();
    }

    if (!(digitalRead(BRS)) && digitalRead(BLS))    // Turn right //Back Right Hall = 0 and Back Left Hall = 1
    {
      Bfronttoright();
      delay(500);
      Brighttofront();
    ...
    if (digitalRead(BRS) && !(digitalRead(BLS)))    // turn left //Back Right Hall = 1 and Back Left Hall = 0
    {
      Bfronttoleft();
      delay(500);
      Blefttofront();
